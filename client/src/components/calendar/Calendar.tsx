...
function EventDialog({ event, isOpen, onClose, onSubmit, onDelete }: EventDialogProps) {
  ...
  const [start, setStart] = useState(event?.start ? format(new Date(event.start), "yyyy-MM-dd'T'HH:mm") : "");
  const [end, setEnd] = useState(event?.end ? format(new Date(event.end), "yyyy-MM-dd'T'HH:mm") : "");
